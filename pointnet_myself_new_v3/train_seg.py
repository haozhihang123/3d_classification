...
from models.utils.ShapeNetDataLoader import ShapeNetDataLoader
from models import seg_models
...
def parse_args():
    ...
    parser.add_argument('--num_workers', default=32, help='num_workers in training')
    parser.add_argument('--model_name', default='PointNet_plus_ssg_seg', help='3d model name')
    ...
    parser.add_argument('--log_dir', default='/nfs/project/myself/pointnet_myself_new_v2/log/', help='dir of log')
    parser.add_argument('--data_dir', default='/nfs/project/myself/shapenetcore_partanno_segmentation_benchmark_v0_normal', help='dir of data')
...
def main(args):
    ...
    # 数据构建
    train_dataset = ShapeNetDataLoader(root=args.data_dir, split='train')
    test_dataset = ShapeNetDataLoader(root=args.data_dir, split='test')
    ...
    # 模型构建
    model = seg_models.__dict__[args.model_name]().to(device)
    ...
    for epoch in range(args.epoch_num):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader),total=len(trainDataLoader)):
            ...
            mean_correct.append(correct.item() /  (args.batch_size * 2500))
            loss.backward() 
            optimizer.step()
            if batch_id % 10 == 0:
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader)):
                ...
                correct = pred_choice.eq(target.long().data).cpu().sum()
                mean_correct.append(correct.item() /  (args.batch_size * 2500))  
            val_accurate = np.mean(mean_correct) 
        ...
        if val_max < val_accurate:
            ...
            torch.save(state, args.log_dir + args.model_name + '/epoch_' + str(epoch) + '_' + str(val_accurate)[:5] + '.pth')
    writer.close()
